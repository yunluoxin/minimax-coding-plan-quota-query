...
- 每天 5 个窗口
- API 返回 `currentIntervalTotalCount`（窗口总量）和 `currentIntervalUsageCount`（窗口剩余量，不是已用量！）
- **重要**：`currentIntervalUsageCount` 是当前窗口**剩余可用次数**，不是已用量
- 窗口用量计算：窗口用量 = `currentIntervalTotalCount` - `currentIntervalUsageCount`
...
class IntervalSnapshot {
    ...
    var usageCount: Int                        // 窗口用量 = totalCount - 剩余量（存储前已计算好）
...
2. 计算窗口用量 = totalCount - usageCount（因为 API 返回的是剩余量）
3. 生成唯一 id: "\(date)_\(intervalIndex)_\(timestamp.timeIntervalSince1970)"
4. 创建 IntervalSnapshot 并存入 SwiftData
5. 调用 cleanupOldData() 清理 30 天前的数据
```
